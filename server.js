...
// app.use(express.json()); //is a built-in mw
// //3rd party mw
// //const kicks = helmet();
// app.use(helmet());
// //app.use(morgan("tiny"));

// //app.use(morgan("dev")); //we comment it oout since we test our own custom mw methodLogger
// //custom mw
// // app.get('/', morgan('dev'));
// // app.gelete('/', morgan('tiny'));
// app.use(methodLogger);
// app.use(addName);
// //app.use(lockout); //but it will lock get request , since it will terminate process by res.status(403).json({ message: "api lockout in force" });

// app.use("/api/hubs", hubsRouter);
// //custom mw
// app.get("/", (req, res) => {
//   //this is inline custom mw
//   const nameInsert = req.name ? ` ${req.name}` : "";
...
//     `);
// });
// //function custom mw, this is like morgan
// //it does not matter where it is defined, it is matter where it is registered:app.use(methodLogger); up
// function methodLogger(req, res, next) {
//   console.log(`${req.method} Request`);
//   //we must add next for respond for preventing infinit loop during request in postman
//   next();
// }
// //will apply to any request
// function addName(req, res, next) {
//   req.name = req.name || "Tatiana";
//   // req.name = req.name || req.headers["x-name"];
//   next();
// }

// function lockout(req, res, next) {
//   let interval = new Date().getSeconds();
//   if (interval % 3 === 0) {
//     res.status(403).json({
//       message: `you shall not pass! api lockout in force every 3 sec: ${interval}`,
//     });
//   } else {
//     next();
//   }
// }
// //when request comes in it is going to go through
// //1.app.use(express.json())
// //2.app.use(helmet());
// //3.app.use(methodLogger);
// //4.app.use(addName);
// //5.app.get("/", (req, res) //here req.name should always has a value, bacause addName mw is registered
// //above of get mw

// //error handlers should be registered last
// //4 parameters signal to Express that this is an error handler mw
// //we use it in hubs-router.js in validateId
// app.use((error, req, res, next) => {
//   res.status(error.code).json({ message: "bad panda from the server!", error });
// });
