...
// // /api/hubs/:id
// //here are two mws
// //it tells: when it is get request with /:id, send validateId mw first, and then send the second one mw:
// //validateId -> next() -> (req,res)
// router.get("/:id", validateId, (req, res) => {
//   res.status(200).json(req.hub);

//   //we do not need the rest one, since we have the same in validateId mw
...
// });
// //-----------MW for this router---------------
// //not all requests needs this mw, this is needed only with :id URI
// function validateId(req, res, next) {
//   const { id } = req.params;
//   Hubs.findById(id)
//     .then((hub) => {
...
//         next();
//       } else {
//         res.statusMessage = "StatusMessage:hub id not found, from validateId";
//         //the next parameter is a signal that it will be used error mw from the server.js
//         next({ code: 404, message: "hub id not found" });
...
// // function requiredBody(req, res, next) {
// //   try{
// //     req.body ?   next() : res.status(400).json({ message: 'please, provide a request body' });
// //   }
// //    catch(err){
...
//     ? next({ message: "please, provide a request body, from requiredBody" })
